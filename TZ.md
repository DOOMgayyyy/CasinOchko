...
1. Термины и сокращения
    ...
    Лобби - Главный экран приложения после авторизации; центральный хаб для навигации по игровым режимам и функциям.
    ...
    ER-модель - Entity-Relationship model - модель данных, описывающая сущности (пользователи, комнаты) и связи между ними.

2. Состояния пользователей

Приложение «КазинОЧКО» определяет четыре основных состояния пользователя, отражающих его текущий контекст взаимодействия с системой. Каждое состояние характеризуется уникальным набором прав, ограничений и визуальных индикаторов интерфейса. Переходы между состояниями происходят автоматически в ответ на игровые события или действия пользователя.

    2.1. Неавторизованный пользователь (гость)
    Неавторизованный пользователь находится на стартовом экране приложения до завершения процедуры входа или регистрации. В этом состоянии система предоставляет минимальный функционал: поля ввода для E-mail и пароля, кнопку «Войти» и возможность перехода к регистрации. Пользователь не имеет доступа к игровым механикам, балансу или личной статистике. Все игровые экраны и функции недоступны до успешной аутентификации. Переход в состояние «Авторизованный пользователь» происходит мгновенно после подтверждения учётных данных сервером или завершения регистрации с начислением стартового бонуса в 5 000 кредитов.

    2.2. Авторизованный пользователь (в лобби)
    После успешной аутентификации пользователь попадает в главное меню - лобби. Это центральное состояние приложения, предоставляющее полный доступ к навигации: выбор режима игры (быстрая игра или приватная комната), просмотр таблицы лидеров, управление профилем через боковое меню и пополнение баланса через рекламу. В лобби отображается текущий баланс кредитов, игровой псевдоним и кнопка вызова меню. Пользователь может свободно перемещаться между разделами лобби, изменять псевдоним или завершать сессию. Переход в игровую комнату инициируется выбором одного из игровых режимов; возврат в это состояние происходит после выхода из комнаты или завершения игровой сессии.

    2.3. Зритель в игровой комнате
    Зритель - это пользователь, находящийся внутри игровой комнаты, но не участвующий в текущем раунде. Состояние присваивается автоматически при входе в любую комнату (общую или приватную) и сохраняется, пока пользователь не сделает ставку не менее 100 кредитов. В режиме зрителя недоступны игровые кнопки «Взять карту» и «Остановиться», а в нижней части экрана отображается информационная надпись «Вы наблюдатель». Пользователь видит игровой процесс других участников, может читать и отправлять сообщения в чат, наблюдать за картами дилера и игроков. Переход в состояние «Игрок» происходит мгновенно после подтверждения ставки >= 100 кредитов. Обратный переход в состояние зрителя наступает при исчерпании баланса ниже минимальной ставки (менее 100 кредитов) в конце раунда или при добровольном выходе из игры без подтверждения новой ставки.

    2.4. Игрок в игровой комнате
    Игрок - это пользователь, подтвердивший ставку не менее 100 кредитов и активно участвующий в текущем раунде блэкджека. В этом состоянии становятся доступными игровые кнопки «Взять карту» и «Остановиться», отображаются личные карты в нижней части экрана, а информационная надпись «Вы наблюдатель» исчезает. Система последовательно активирует ход каждого игрока, предоставляя 15 секунд на принятие решения. По завершении раунда баланс пользователя обновляется согласно результатам. После подсчёта результатов пользователь возвращается в фазу ставок: при балансе >= 100 кредитов сохраняет статус игрока с возможностью подтвердить новую ставку; при балансе < 100 кредитов автоматически переводится в статус зрителя до пополнения счёта.
...
4. Требования к экранам приложения
    ...
    4.4. Экраны игровой комнаты (Общая комната, приватная комната, комната зрителя)
        ...
        * Центральная часть экрана (игровой стол):
            ...
            * Поле "Таймер".
            * Поле "Очки диллера".
            * Поле "Ставка". Поле содержит в себе информацию о счете игрока, о размере ставки, кнопки увеличения/уменьшения ставки на 50 монет.
        ...
        * После создания приватной комнаты появляется модальное окно с сообщением "Комната создана!" и уникальным 4-значным кодом комнаты. Это окно отображается только при создании комнаты, код комнаты можно скопировать нажатием на него.   
        ...
        4.4.5. Функциональные требования
            ...
            * Для активного игрока должны быть доступны действия: "Взять карту", "Отказаться"
            * Система должна предоставлять 15 секунд на принятие решения каждым игроком
            * При превышении времени система должна автоматически выбирать действие "Отказаться"
            * После завершения ходов игроков система должна автоматически играть за дилера по правилам
            * Система должна рассчитывать результаты согласно правилам выплат (согласно "Rules")
            ...
            * При балансе менее 100 кредитов игрок должен автоматически переходить в статус "Зритель"
            ...
            4.4.5.3. Бизнес-логика навигации
            * При нажатии кнопки "←" система должна отображать модальное окно подтверждения выхода
            ...
            * В приватной комнате система должна отображать модальное окно с кодом комнаты
    ...
    4.5. Модальные окна
        ...
        4.5.5. Модальное окно подтверждения выхода

            4.5.5.1. Общее описание:
            Модальное окно подтверждения выхода появляется при нажатии кнопки "←" (выход из игры) на игровом экране. Окно предназначено для предотвращения случайного выхода из комнаты и запроса подтверждения у пользователя перед завершением игровой сессии.

            4.5.5.2. Визуальное оформление:
            * Фон: Полупрозрачный темный оверлей, затемняющий основной экран.
            * Контейнер: Центральная панель с закругленными углами в минималистичном стиле, соответствующем общему дизайну игры.
            * Заголовок: Стилизованная надпись "Покинуть комнату?" в верхней части контейнера.
            * Кнопки: Две кнопки в нижней части контейнера:
            * Кнопка "ПОКИНУТЬ" (красного цвета)
            * Кнопка "ОСТАТЬСЯ" (зеленого цвета)

            4.5.5.3. Структура и содержимое:
            * Заголовок: "Покинуть комнату?"
            * Две кнопки с четким разделением по цвету и функционалу:
            * Красная кнопка "ПОКИНУТЬ" - подтверждает выход из комнаты
            * Зеленая кнопка "ОСТАТЬСЯ" - отменяет действие и возвращает пользователя в игру

            4.5.5.4. Функциональные требования:
            * При нажатии кнопки "←" на игровом экране должно появиться модальное окно подтверждения выхода.
            * При нажатии кнопки "ПОКИНУТЬ" система должна:
            * Если пользователь находится в активном раунде - списать текущую ставку
            * Перенаправить пользователя в лобби
            * При нажатии кнопки "ОСТАТЬСЯ" система должна закрыть модальное окно и вернуть пользователя в игру.
            * Модальное окно должно блокировать основной интерфейс до принятия решения пользователем.
            * При выходе во время активного раунда система должна списывать текущую ставку.
