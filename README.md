...
### Сервер
Из папки server копируем код в `C://OSPanel/home` (для 6-й версии) или в `C://OSPanel/domains` (для более старых версий), или настраиваем директорию в OSPanel/MAMP.

Папку называем так, как будет называться наш домен. Например: ``casinochko``.

Перезапускаем OSPanel/MAMP.

Сервер доступен по адресу, указанному в названии папки. Протокол по умолчанию: ``http://``. То есть общий адрес сервера: ``http://casinochko`` (или ``http://casinochko:81`` для MAMP/OpenServer с нестандартным портом).
...
В ``client\src\config.ts`` в константе ``HOST`` прописываем название папки плюс протокол и путь до API. 

Доступные варианты настройки:
- Для OpenServer 6.4.0: ``http://casinochko.local/api``
- Для OpenServer (старые версии): ``http://casinochko/api``
- Для практики/тестирования: ``http://casinochko:81/api``
- Для MAMP: ``http://localhost:8888/server/api``

То есть в данном случае папка в OSPanel/MAMP называется ``casinochko`` (или ``casinochko.local``). 
...
### База данных
Базу данных назвать аналогично названию проекта (например: ``casinochko``).
...
В таблице `messages` - сообщения чата комнат.

В таблице `message_hashes` лежат хеши чатов комнат. Для каждой комнаты содержится **`ОДНА`** запись. Это необходимо для непротиворечивого обращения к хешам чата в системе.

В таблице `rooms` хранятся игровые комнаты (открытые и приватные).

В таблице `room_members` хранятся участники комнат (игроки и зрители).
