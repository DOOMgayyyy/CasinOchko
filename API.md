...
4.16. leaveRoom
Покинуть комнату. Удаляет пользователя из комнаты, возвращает ставку и обновляет состояние.
Параметры
{
    token: string; - токен авторизации
}
...
Примечание: 
- Если у игрока была ставка, она автоматически возвращается на баланс
