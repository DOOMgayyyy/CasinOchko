...
| getInfoRoom | Получить информацию о комнате |
| leaveRoom | Покинуть комнату |
| takeUserCard | Взять карту из колоды |
...
903 - Участник комнаты не найден / Не удалось покинуть комнату
...
4.17. takeUserCard
Взять карту из колоды. Игрок получает одну карту из колоды комнаты. Автоматически передает ход следующему игроку при переборе (сумма > 21).
...
Примечание: 
- Можно взять только если ваш ход (current_member_id совпадает с вашим member_id)
- Зрители (status: 'spectator') не могут брать карты
- Максимум 5 карт в руке по правилам игры
- При переборе (сумма > 21) ход автоматически передается следующему игроку
- После того как все игроки завершат ход, автоматически начинается ход дилера
- Обновляется hash комнаты для синхронизации всех клиентов
...
* 908 - Зритель не может брать карты (статус spectator)
* 909 - Не ваш ход (current_member_id не совпадает)
* 910 - Максимум 5 карт в руке